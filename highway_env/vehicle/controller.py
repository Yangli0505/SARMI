...
from highway_env.vehicle.trajectory_planning import polynomial_5order, MPCSOLVE_IPOPT
...
class ControlledVehicle(Vehicle):
    ...
    def mpc_control(self, target_speed: float, target_lane_index: LaneIndex):
        ...
        # get road boundary
        road_min_x, road_max_x = float('inf'), float('-inf')
        road_min_y, road_max_y = float('inf'), float('-inf')
        current_lane = self.road.network.get_lane(self.lane_index)
        for lane in target_lane, current_lane:
            start_position = lane.position(0, 0)          # 获取车道起点坐标
            end_position = lane.position(target_lane.length, 0)  # 获取车道终点坐标
            lane_width = lane.width_at(0)
            min_x = min(start_position[0], end_position[0])
            max_x = max(start_position[0], end_position[0])
            min_y = min(start_position[1] - lane_width/2, end_position[1] - lane_width/2)
            max_y = max(start_position[1] + lane_width/2, end_position[1] + lane_width/2)
            road_min_x = min(road_min_x, min_x)
            road_max_x = max(road_max_x, max_x)
            road_min_y = min(road_min_y, min_y)
            road_max_y = max(road_max_y, max_y)
        road_boundary = {
            'min_x': road_min_x,
            'max_x': road_max_x,
            'min_y': road_min_y,
        ...
        # get surrounding cars
        sur_cars_info = self.road.vehicles[1:]  # 获取周围车辆

        # future reference
        future_N = 10
        state_n = 4
        future_ref = np.mat(np.zeros([state_n * future_N, 1]))  # 全局坐标
        delta_dis = target_speed * 0.1
        for i in range(future_N):
            future_ref[i * state_n, 0] = target_lane.position(local_position_ref + delta_dis * (i+1), 0.)[0]      # 笛卡尔坐标系横坐标
            future_ref[i * state_n + 1, 0] = target_lane.position(local_position_ref + delta_dis * (i+1), 0.)[1]  # 笛卡尔坐标系纵坐标
            future_ref[i * state_n + 2, 0] = target_speed
            future_ref[i * state_n + 3, 0] = target_lane.heading_at(local_position_ref + delta_dis * (i+1))
        
        # MPC setting
        rollout_N = 10
        mpc = MPCSOLVE(rollout_space=rollout_N)
        P, Q, G, H = mpc.compute_matrix(car_info,ref_info,future_ref, road_boundary, sur_cars_info)
        ...
        self.future_state = future_ref + Y[0:future_N * mpc.state_n, 0]  # 全局坐标
        # print(self.future_state)
        
        return a, theta
##################################################################################################################################################
        # # get the target in the Frenet
        # target_lane = self.road.network.get_lane(target_lane_index)
        # target_local_position = [target_lane.local_coordinates(self.position)[0], 0.]
        # delta_dis = target_speed * 0.1
        # x_traj   = np.zeros(self.planning_step + 1)
        # y_traj   = np.zeros(self.planning_step + 1)
        # v_traj   = np.zeros(self.planning_step + 1)
        # fai_traj = np.zeros(self.planning_step + 1)
        # for i in range(self.planning_step + 1):
        #     x_traj[i] = target_lane.position(target_local_position[0] + delta_dis * i, target_local_position[1])[0]  # 笛卡尔坐标系横坐标
        #     y_traj[i] = target_lane.position(target_local_position[0] + delta_dis * i, target_local_position[1])[1]  # 笛卡尔坐标系纵坐标
        #     v_traj[i] = target_speed
        #     fai_traj[i] = target_lane.heading_at(target_local_position[0] + delta_dis * i)
        # trajectory_init = {
        #     'x': x_traj,
        #     'y': y_traj,
        #     'v': v_traj,
        #     'fai': fai_traj,
        # }
        # # print(trajectory_init)

        # # get the current in the Frenet
        # current_local_position = target_lane.local_coordinates(self.position)
        ...
        # # planning:traj opt
        # mpc = MPCSOLVE_IPOPT(rollout_space=self.planning_step)
        # X_sol, U_sol = mpc.solve_nmpc(trajectory_init, ego_car_info, sur_cars_info, road_boundary)
        # # print(X_sol)
        # # print(U_sol)
        # a = U_sol[0,0]
        # theta = U_sol[1,0]
        # self.future_state = np.mat(X_sol).flatten('F').reshape(-1, 1)
        # self.future_state = self.future_state[-40:]

        # # 绘制x和y轨迹
        # trajectory = np.mat(X_sol)
        # x_vals = np.array(trajectory[0, :]).flatten()
        # y_vals = np.array(trajectory[1, :]).flatten()
        # plt.figure(figsize=(12, 6))
        # plt.subplot(1, 2, 1)
        # plt.plot(trajectory_init['x'], trajectory_init['y'], label='Position (x, y)')
        # plt.plot(x_vals, y_vals, label='Optimal Trajectory (x, y)', linestyle='--')  # 添加不同的线型
