...
class safe_check:
    ...
    def collision_check(self, action:int) -> bool:
        ...
        collision_other, collision_object, close_to_vehicle = False, False, False
        ...
        if self.ego.position[0] < self.road.road_ends and self.ego.position[0] > 630:
            for i in range(len(self.road.vehicles) - 1):
                ...
                # 贴的太近有碰撞风险
        for i in range(len(self.road.vehicles) - 1):
            if self.road.vehicles[i].lane_index == self.ego.lane_index:
                delta_dis = self.ego.lane_distance_to(self.road.vehicles[i])
                if delta_dis < 10 and delta_dis > 0:
                    close_to_vehicle = True
                    
        unsafe_action = [collision_other, collision_object, close_to_vehicle]
