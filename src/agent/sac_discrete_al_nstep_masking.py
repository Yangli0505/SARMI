...
class ReplayBuffer:
    ...
    def __init__(self, buffer_size, batch_size, train_steps=4000, initial_beta=0.4, n_step=3,
        ...
        self.experience = namedtuple("Experience",
                                     field_names=["state", "action", "reward", "next_state", "done", "cost", "action_mask"])
    ...
    def add(self, state, action, reward, next_state, done, cost, action_mask=None):
        ...
        if self.n_step != 1:
            ...
            if self.n_step_buff.is_full():
                ...
                e = self.experience(state, action, reward, next_state, done, cost, action_mask)
    ...
    def sample(self):
        ...
        if experiences[0].action_mask is not None:
            action_masks = torch.from_numpy(np.vstack([e.action_mask for e in experiences if e is not None])).float()
            
        return (states, actions, rewards, next_states, dones, cost, action_masks), tree_idx, ISweight
...
class SAC(nn.Module):
    ...
    def __init__(self,
        ...
        # Augmented Lagrangian 相关参数
        self.sigma = 2e-5  # 初始惩罚因子 0.1 1e-4
        self.scale_factor = 1.01  # 比例因子ρ 1.05
        self.sigma_max = 10.0  # 惩罚因子上限 5
        self.update_sigma_freq = config.update_sigma_freq  # 更新周期
        self.sigma_step_counter = 0  # 计步器

        # 添加action masking相关属性
        self.use_action_masking = config.use_action_masking
        self.action_size = action_size

    def get_action_mask(self, state, ego, road):
        """根据状态生成动作掩码"""
        # 0: 'LANE_LEFT',
        # 1: 'IDLE',
        # 2: 'LANE_RIGHT',
        ...
        mask = np.ones(self.action_size) # 初始化掩码,所有动作都允许

        # 场景一：匝道禁止变道
        if (ego.position[1] > 7.5) and (ego.position[1] < 20):
            mask[0] = 0
            mask[2] = 0
        ...
        # 场景三：加速车道禁止向右变道
        if ego.position[1] > 2.5 and ego.position[1] < 7.5:
            mask[2] = 0
    ...
    def get_action(self, state, ego, road):
        """Returns actions for given state as per current policy."""
        state = torch.from_numpy(state).float()
        
        # 如果启用了masking,则生成mask
        if self.use_action_masking:
            action_mask = self.get_action_mask(state, ego, road)
            action_mask = torch.from_numpy(action_mask).float()
            # 扩展维度以匹配batch
            action_mask = action_mask.unsqueeze(0)
            # 将action_mask传递给actor的evaluate方法
            action, _, _ = self.actor_local.evaluate(state.unsqueeze(0), action_mask)
            return action.numpy()[0]
        else:
            action, _, _, _ = self.actor_local.step(state)
            return action.numpy()

    def calc_policy_loss(self, states, ISweight, alpha, action_masks=None):
        """Calculate policy loss using Augmented Lagrangian method"""
        if action_masks is not None and self.use_action_masking:
            _, action_probs, log_pis = self.actor_local.evaluate(states, action_masks)
        else:
            _, action_probs, log_pis = self.actor_local.evaluate(states)

        # Get Q-values and cost estimates
        q1 = self.critic1(states)
        q2 = self.critic2(states)
        qc = self.cost(states)
        min_Q = torch.min(q1, q2)

        # Calculate cost constraint and violation
        cost_constraint = self.cost_limit * (1 - self.gamma ** self.max_ep_len) / (1 - self.gamma) / self.max_ep_len
        cost_value = (action_probs * qc).sum(1)  # Expected cost
        constraint_violation = cost_value - cost_constraint
        # print(cost_value)
        ...
        lambda_term = self.penalty * constraint_violation  # 一阶惩罚项
        quadratic_term = 0.5 * self.sigma * (constraint_violation ** 2)  # 二阶惩罚项
        
        # Compute total policy loss
        actor_loss = (action_probs * (alpha * log_pis - min_Q) * ISweight).sum(1).mean() + \
                     (lambda_term - damp).mean() + quadratic_term.mean()
                 
        # Update sigma periodically
        self.sigma_step_counter += 1
        if self.sigma_step_counter >= self.update_sigma_freq:
            self.sigma = min(self.sigma * self.scale_factor, self.sigma_max)
            self.sigma_step_counter = 0
                 
        log_action_pi = -torch.sum(log_pis * action_probs, dim=1)
        return actor_loss, log_action_pi

    def learn(self, experiences, gamma, ISweight, logger=None, ego=None, road=None):
        ...
        """
        states, actions, rewards, next_states, dones, cost, action_masks = experiences

        # 如果启用了masking且没有提供action_masks,则为每个状态生成mask
        if self.use_action_masking and action_masks is None:
            action_masks = torch.stack([torch.from_numpy(self.get_action_mask(s, ego, road)) for s in states.numpy()]).float()

        # ---------------------------- loss actor ---------------------------- #
        current_alpha = copy.deepcopy(self.alpha)
        actor_loss, entropy = self.calc_policy_loss(states, ISweight, current_alpha, action_masks)

        # Compute alpha loss
        if self.use_action_masking:
            # 初始化 target_entropy_temp
            target_entropy_temp = torch.zeros(entropy.shape[0], dtype=entropy.dtype, device=entropy.device)
            # 遍历 action_masks，逐元素更新 target_entropy_temp
            for idx, action_mask in enumerate(action_masks):
                if action_mask is None:
                    target_entropy_temp[idx] = self.target_entropy  # 对应位置填充 self.target_entropy
                else:
                    size = torch.sum(action_mask == 1)  # 当前样本中值为 1 的数量
                    target_entropy_temp[idx] = -size  # 对应位置更新为 -size
            # 计算 alpha_loss
            alpha_loss = - (self.log_alpha * (target_entropy_temp.detach() - entropy.detach()).detach() * ISweight).mean()
        else:
            # 默认情况直接计算 alpha_loss
        ...
        with torch.no_grad():
            if action_masks is not None and self.use_action_masking:
                _, action_probs, log_pis = self.actor_local.evaluate(next_states, action_masks)
            else:
        ...
        constraint_violation = qc.detach() - cost_constraint
        # penalty_loss = -(self.penalty * (constraint_violation * ISweight).mean() + \
        #                 0.5 * self.sigma * ((constraint_violation ** 2) * ISweight).mean())
        penalty_loss = -self.penalty * (constraint_violation * ISweight).mean()
        ...
        # penalty
        self.penalty_optimizer = optim.Adam(params=[self.penalty], lr=self.sigma)
        ...
        logger.store(LossPi=actor_loss.item(), alpha_loss=alpha_loss.item(), critic1_loss=critic1_loss.item(),
                     ...
                     cost=qc.mean().item(), entropy=entropy.mean().item(), sigma=self.sigma)
...
def train(config, logger_kwargs=dict()):
    ...
    with wandb.init(project=config.proj_name, name=config.run_name, config=config):
        ...
        while steps < int(config.num_steps / num_procs()):
            ego = env.vehicle
            road = env.road
            action = agent.get_action(state, ego, road)

            if re_action:
                action_origin = action
                # "simple" or "mpc"
                action = env.check_action(int(action), "mpc")
            ...
            if steps % config.update_freq == 0:
                for _ in range(config.update_freq):
                    ...
                    abs_error = agent.learn(
                        experiences, gamma=0.99, ISweight=ISweight, logger=logger, ego=ego, road=road)
                    if config.per:
            ...
            if steps % local_steps_per_epoch == 0:
                ...
                wandb.log({"AverageRewards": logger.get_stats('EpRet')[0],
                           ...
                           "Sigma": logger.get_stats('sigma')[0],
                           "Steps": mpi_sum(steps),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--proj_name", type=str, default="Augmented_Lagrangian_MPC")
    ...
    parser.add_argument('--update_sigma_freq', type=int, default=1000, 
                        help='Update frequency for penalty factor sigma')
    parser.add_argument('--max_ep_len', type=int, default=1000)
    parser.add_argument('--cpu', type=int, default=4)
    parser.add_argument('--per', type=bool, default=False) # True False
    parser.add_argument('--n_step', type=int, default=3, help='1 or 3')
    parser.add_argument('--cost_limit', type=float, default=1.5)  # 0.01   0.1                                                                                                                                                                                                               
    parser.add_argument("--use_action_masking", type=bool, default=True,  
                        help="Whether to use action masking, default: True") # True False
    args = parser.parse_args()
    
    if args.safe_check:
        args.proj_name = "SARMI"
