...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, done, cost, action_mask=None):
        ...
        if self.n_step != 1:
            ...
            e = self.experience(state, action, reward, next_state, done, cost, action_mask)
            initial_priority = 1.0  # 新样本使用最大优先级
            
            if self.memory_counter < self.size:
                self.memory.append(e)
                self.tree.add(initial_priority)
            else:
    ...
    def sample(self):
        ...
        if self.use_per:
            ...
            total_p = max(self.tree.total_p, 1e-8)  # 避免除零
            priority_segment = total_p / self.batch_size
            
            # 计算最小优先级
            priorities = self.tree.tree[-self.tree.capacity:]
            positive_priorities = priorities[priorities > 0]
            min_priority = max(np.min(positive_priorities) if len(positive_priorities) > 0 else 1e-8, 1e-8)
            ...
            for i in range(self.batch_size):
                ...
                # 确保data_idx在有效范围内
                if data_idx >= len(self.memory):
                    data_idx = len(self.memory) - 1
                
                # 安全处理
                priority = max(priority, 1e-8)
                prob = priority / total_p
                ISweight[i] = np.power(prob / (min_priority / total_p), -self.beta)
                
                experiences.append(self.memory[data_idx])
                tree_idx[i] = idx
            
            # 归一化IS权重
            ISweight = ISweight / max(ISweight.max(), 1e-8)
...
class SAC(nn.Module):
    ...
    def __init__(self,
        ...
        # Augmented Lagrangian 相关数
        self.sigma = 5e-2  # 初始惩罚因子
        self.scale_factor = 1.05  # 比例因子ρ
        self.sigma_max = 5.0  # 惩罚因子上限
    ...
    def learn(self, experiences, gamma, ISweight, logger=None, ego=None, road=None):
        ...
        # 使用较小的Q值来计算TD误差
        td_error = torch.min(torch.abs(q1 - Q_targets), torch.abs(q2 - Q_targets))
        abs_error = td_error.detach().cpu().numpy()
        abs_error = np.clip(abs_error, 1e-8, 1.0)  # 限制TD误差范���
...
if __name__ == "__main__":
    ...
    parser.add_argument("--proj_name", type=str, default="Augmented_Lagrangian")
    ...
    parser.add_argument('--per', type=bool, default=True) # True False
    ...
    if args.safe_check:
        args.proj_name = "Augmented_Lagrangian_MPC"
