...
def cpo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
        ...
        penalty_lr=5e-2, damping=0.5, config=None):
    """
    Constrained Policy Optimization (based on PPO with Lagrangian for constraints)
    """
    ...
    def update(re_action):
        ...
        loss_penalty = -penalty_param * cost_deviation
        ...
        cv_l_old = compute_loss_vc(data).item()
        ...
        logger.store(LossPi=pi_l_old, LossV=v_l_old, LossCV=cv_l_old,
                     KL=kl, Entropy=ent, ClipFrac=cf,
                     DeltaLossPi=(loss_pi.item() - pi_l_old),
                     DeltaLossV=(loss_v.item() - v_l_old),
                     DeltaLossCV=(loss_vc.item() - cv_l_old))
    ...
    with wandb.init(project=config.proj_name, name=config.run_name, config=config):
        ...
        for epoch in range(epochs):
            ...
            logger.log_tabular('DeltaLossCV', average_only=True)
...
if __name__ == '__main__':
    ...
    cpo(lambda: gym.make(args.env), actor_critic=core.MLPActorCritic,
        ...
        re_action=args.safe_check, penalty_lr=args.penalty_lr, damping=args.damping, logger_kwargs=logger_kwargs, config=args)